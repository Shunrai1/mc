...
export default class Renderer {
  ...
  setPostProcess() {
    ...
    const bloomResolution = new THREE.Vector2(Math.floor(this.sizes.width / 2), Math.floor(this.sizes.height / 2))
  ...
  debugInit() {
    ...
    bloomFolder
      .addBinding(this.postProcessConfig.bloom, 'enabled', {
        label: '启用',
      })
      .on('change', (ev) => {
    ...
    bloomFolder
      .addBinding(this.postProcessConfig.bloom, 'strength', {
        label: '强度',
        min: 0,
        max: 3,
        step: 0.01,
    ...
    bloomFolder
      .addBinding(this.postProcessConfig.bloom, 'radius', {
        label: '半径',
        min: 0,
        max: 1,
        step: 0.01,
    ...
    bloomFolder
      .addBinding(this.postProcessConfig.bloom, 'threshold', {
        label: '阈值',
        min: 0,
        max: 1,
        step: 0.01,
    ...
    speedLinesFolder
      .addBinding(this.postProcessConfig.speedLines, 'enabled', {
        label: '启用',
      })
      .on('change', (ev) => {
    ...
    speedLinesFolder
      .addBinding(this.postProcessConfig.speedLines, 'color', {
        label: '颜色',
        view: 'color',
      })
      .on('change', (ev) => {
        this.speedLinePass.uniforms.uColor.value.setRGB(ev.value.r / 255, ev.value.g / 255, ev.value.b / 255)
      })

    speedLinesFolder
      .addBinding(this.postProcessConfig.speedLines, 'density', {
        label: '密度',
        min: 10,
        max: 100,
        step: 1,
    ...
    speedLinesFolder
      .addBinding(this.postProcessConfig.speedLines, 'speed', {
        label: '脉冲速度',
        min: 0.5,
        max: 10,
        step: 0.1,
    ...
    speedLinesFolder
      .addBinding(this.postProcessConfig.speedLines, 'thickness', {
        label: '三角形宽度',
        min: 0.01,
        max: 0.5,
        step: 0.01,
    ...
    speedLinesFolder
      .addBinding(this.postProcessConfig.speedLines, 'minRadius', {
        label: '尖端半径',
        min: 0.1,
        max: 0.8,
        step: 0.01,
    ...
    speedLinesFolder
      .addBinding(this.postProcessConfig.speedLines, 'maxRadius', {
        label: '起始半径',
        min: 0.8,
        max: 2.0,
        step: 0.01,
    ...
    speedLinesFolder
      .addBinding(this.postProcessConfig.speedLines, 'randomness', {
        label: '随机性',
        min: 0,
        max: 1,
        step: 0.01,
    ...
    gazeFolder
      .addBinding(this.postProcessConfig.gaze, 'enabled', {
        label: '启用',
      })
      .on('change', (ev) => {
    ...
    gazeFolder
      .addBinding(this.postProcessConfig.gaze, 'intensity', {
        label: '强度',
        min: 0,
        max: 1,
        step: 0.01,
    ...
    shadowFolder
      .addBinding(SHADOW_CONFIG, 'quality', {
        label: '质量等级',
        options: {
  ...
  _renderPlayerPreview() {
    if (!this.playerPreview?.enabled) return
