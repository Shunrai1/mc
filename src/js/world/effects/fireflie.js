...
export default class Fireflies {
  ...
  _updateStreaming(playerPos) {
    ...
    if (pcx === this._lastPlayerCellX && pcz === this._lastPlayerCellZ) return
  ...
  _getNightFactor() {
    ...
    if (!environment?.dayCycle) return 0

    const t = environment.dayCycle.params.timeOfDay

    if (t >= 0.28 && t <= 0.78) return 0
    if (t > 0.78 && t < 0.85) {
      const f = (t - 0.78) / 0.07
      return f * f * (3 - 2 * f) // smoothstep
    }
    if (t >= 0.85 || t < 0.22) return 1
    if (t >= 0.22 && t < 0.28) {
  ...
  debugInit() {
    ...
    this.debugFolder
      .addBinding(this.params, 'countPerCell', {
        label: 'Count / Cell',
        min: 5,
        max: 100,
        step: 5,
    ...
    this.debugFolder
      .addBinding(this.params, 'viewRadius', {
        label: 'View Radius',
        min: 1,
        max: 6,
        step: 1,
    ...
    this.debugFolder
      .addBinding(this.params, 'spawnHeight', {
        label: 'Spawn Height',
        min: 2,
        max: 20,
        step: 1,
