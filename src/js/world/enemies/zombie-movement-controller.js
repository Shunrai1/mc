...
// 控制僵尸的移动、AI 状态转换、碰撞和攻击逻辑。
// 1. 计算到玩家方向和距离
// 2. 击退处理（knockbackTimer > 0 时覆盖移动）
// 3. AI 状态转换（距离 + 冷却 → 决定新状态）
// 4. 根据状态设置速度和朝向
// 5. 重力
// 6. 碰撞检测（复用 EntityCollisionSystem）
// 7. 障碍跳跃（移动中 + 碰撞候选多 → 自动跳）
// 8. 异常检测（碰撞过多 / 掉出世界 → 标记重生）
  ...
  update(playerPos, currentState) {
    ...
    if (this.knockbackTimer > 0) {
      ...
      if (newState === ZombieState.CHASE) {
        ...
        if (
          (newState === ZombieState.ATTACK ||
            (newState === ZombieState.IDLE && distanceToPlayer <= this.ATTACK_RANGE)) &&
          distanceToPlayer > 0
        ) {
          const angle = Math.atan2(directionToPlayer.x, directionToPlayer.z)
          this.group.rotation.y = angle
