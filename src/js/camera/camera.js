...
export default class Camera {
  ...
  toggleSide() {
    // 仅在第三人称模式下切换左右
    if (this.currentMode !== this.cameraModes.THIRD_PERSON || !this.rig) return
  ...
  setInstances() {
    ...
    this.perspectiveCamera = new THREE.PerspectiveCamera(55, this.sizes.width / this.sizes.height, 0.1, 512)
   ...
   */
  switchMode(mode) {
    if (!Object.values(this.cameraModes).includes(mode) || mode === this.currentMode) return
  ...
  _notifyRenderer() {
    if (this.rendererRef?.onCameraSwitched) {
      // onCameraSwitched 方法，让渲染器知道相机已切换，以便进行相应的调整（如更新阴影、视锥体裁剪等）
  ...
  setDebug() {
    if (this.debugActive) {
      ...
      this.debugFolder
        .addBinding(controlsToggle, 'useOrbitControls', {
      ...
      // 相机视锥助手
      this.debugFolder
        .addBinding(this, 'cameraHelperVisible', {
          label: '显示相机助手',
        })
        .on('change', (ev) => {
          if (this.cameraHelper) {
  ...
  _applyTerrainAdaptation(desiredCameraPos) {
    ...
    const targetY = Math.min(Math.max(desiredCameraPos.y, minY), desiredCameraPos.y + this.terrainAdapt.maxRaise)
