...
export default class CameraRig {
  ...
  _setupEventListeners() {
    ...
    emitter.on('input:mouse_move', ({ movementY }) => {
      ...
      const sensitivityMultiplier =
        this.isTelescopeActive && this.config.trackingShot.telescope?.enabled
  ...
  setShoulderMode(mode) {
    if (this._shoulderMode === mode) return
    this._shoulderMode = mode

    const targetSide = mode === 'right' ? 1 : mode === 'left' ? -1.25 : 0
    const targetHeight =
      mode === 'center' ? (this.config.centerElevated ? this.config.centerElevated.heightBoost : 0) : 0
  ...
  _updateBobbing(speed, isMoving) {
    ...
    if (isMoving && speed > 0.1) {
      ...
      const verticalOffset =
        Math.sin(elapsed * bobbing.verticalFrequency * Math.PI * 2) * bobbing.verticalAmplitude * speedFactor

      // 水平震动 (轻微左右摆动)
      const horizontalOffset =
        Math.sin(elapsed * bobbing.horizontalFrequency * Math.PI * 2) * bobbing.horizontalAmplitude * speedFactor

      // Roll 倾斜 (模拟左右脚步的重心转移)
      this._bobbingRoll = Math.sin(elapsed * bobbing.rollFrequency * Math.PI * 2) * bobbing.rollAmplitude * speedFactor

      this._bobbingOffset.set(horizontalOffset, verticalOffset, 0)
    } else if (bobbing.idleBreathing.enabled) {
      // 静止时的呼吸感
      const breathingOffset =
        Math.sin(elapsed * bobbing.idleBreathing.frequency * Math.PI * 2) * bobbing.idleBreathing.amplitude
  ...
  setDebug(debugFolder) {
    ...
    visualFolder
      .addBinding({ showHelpers: this.helpersVisible }, 'showHelpers', {
        label: '显示锚点助手',
      })
      .on('change', (ev) => {
